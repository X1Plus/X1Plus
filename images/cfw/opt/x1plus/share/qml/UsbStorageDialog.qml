...
Item {
    ...
    property var buttons: SimpleItemModel {
        DialogButtonItem {
            name: "print"
            title: printStatus !== "" ? printStatus : qsTr("Print")
            ...
            onClicked: {
                X1Plus.DDS.publish("device/request/print", {
                    command: "gcode_file",
                    param: currentPath + "/" + selectedEntry.name,
                    sequence_id: 0
                });
                printStatus = qsTr("Print started!");
                printResetTimer.start();
            }
        }
        DialogButtonItem {
            name: "copy"
